...
void lzw_encode(char *text, Dictionary *dict)
{
    ...
    while (i < strlen(text))
    {
        ...
        next = text[i + 1];              //C = next input character 下一個

        while (get_seq_code(dict, current) != NOT_EXIST) //IF P + C is in the string table  若字母在字典裡面找到current的話
        {
            sprintf(current, "%s%c", current, next); //P= P+C 新組合放入 current
            i++;                                     //next 下一個
            next = text[i + 1];                      // 探測 next ＝下一個next 何時停？
        }

        current[strlen(current) - 1] = '\0'; //若字母在字典裡面找不到current的話 必須往後探測一位  往前推一個格子 TOB 把B去掉 current 最後一個變成 No byte
        code = get_seq_code(dict, current);  //output the code for P  把current 編碼下來
        next = text[i];                      //p=c  next 要縮一格回去 因為 上面有next = text[i+1];
        if (code != NOT_EXIST)
            j++;
        ...
        insert_seq(dict, current);               // add P + C to the string table 新組合current放入字典裡面編碼

        //25 bytes 壓縮到-> 16 bytes
    }
    printf("=================================\n");
...
//         {                                            //IF P + C is in the string table  若字母在字典裡面找到current的話
//             sprintf(current, "%s%c", current, next); //P= P+C 新組合放入 current
//             i++;                                     //next 下一個
//             next = text[i + 1];                      // 探測 next ＝下一個next 何時停？
//             if (next == '\0')
...
//         printf("%d%7c", code, ' ');
//         sprintf(current, "%s%c", current, next); //新組合
//         insert_seq(dict, current);               // add P + C to the string table 新組合current放入字典裡面
//                                                  //輸出
//         //25 bytes 壓縮到-> 167 bytes
//     }
//     printf("=================================\n");
//     ans = ((float)(strlen(inputText) - j) / strlen(inputText)) * 100;
//     printf("Input Text is %s: total %lu bytes -> coded %u bytes   Data reduction rate : %1.f%%  \n", inputText, strlen(inputText), j, ans);
